<template>
    <div>
        ...
        <div class="mt-2">
            <span
                v-for="(word, index) in getWordDiff(tagsToList(revision.tags) || '', tagsToList(currentTags) || '')"
                :key="index"
                :class="`text-${getDiffColor(word)}`"
            >
                {{ word.value }}
        ...
        <div class="mt-2">
            <span
                v-for="(line, index) in getLineDiff(new Date(revision.published_at).toDateString() || '', new Date(form.published_at).toDateString() || '')"
                :key="index"
                :class="`text-${getDiffColor(line)}`"
            >
                {{ line.value }}
